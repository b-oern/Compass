...
package net.bsnx.apps.compass.preference
...
class PreferenceStore(context: Context, lifecycle: Lifecycle) {
    ...
    private fun updateNightMode() {
        val storedNightMode = sharedPreferences
            ...
            ?.let { AppNightMode.Companion.forPreferenceValue(it) }
    ...
    fun setStepCount(stepCount: Int) {
        sharedPreferences.edit().putInt("stepCount", stepCount).commit()
    }

    fun getStepCount(defaultValue : Int = 0) : Int {
        return sharedPreferences.getInt("stepCount", defaultValue)
    }
}
